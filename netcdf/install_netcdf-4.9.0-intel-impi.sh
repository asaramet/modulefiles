...
TARGET_DIR="${BASE_DIR}/${VERSION}-intel-2021.4-impi-2021.4"
...
module load lib/hdf5/1.12.2-intel-2021.4.0-impi-2021.4.0
...
cd netcdf-c-${VERSION}
CC=icc CFLAGS="-I${HDF5_INC_DIR}" LDFLAGS="-L${HDF5_LIB_DIR}" ./configure --prefix=${TARGET_DIR} --with-mpiexec=mpiicc
make check
make install
...
CC=icc CPPFLAGS="-I${HDF5_INC_DIR} -I${TARGET_DIR}/include" LDFLAGS="-L${HDF5_LIB_DIR} -L${TARGET_DIR}/lib" ./configure --prefix=${TARGET_DIR} --disable-filter-testing 
make check
make install
...
FC=ifort CPPFLAGS="-I${HDF5_INC_DIR} -I${TARGET_DIR}/include" LDFLAGS="-L${HDF5_LIB_DIR} -L${TARGET_DIR}/lib" ./configure --prefix=${TARGET_DIR} --disable-largefile --with-mpiexec=mpiifort
make check
make install
