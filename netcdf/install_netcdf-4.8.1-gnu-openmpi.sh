...
CC=mpicc CPPFLAGS="-I${HDF5_INC_DIR} -I${TARGET_DIR}/include" LDFLAGS="-L${HDF5_LIB_DIR} -L${TARGET_DIR}/lib64" ./configure --prefix=${TARGET_DIR} --disable-filter-testing
make check
make install
...
FC=mpifort CPPFLAGS="-I${HDF5_INC_DIR} -I${TARGET_DIR}/include" LDFLAGS="-L${HDF5_LIB_DIR} -L${TARGET_DIR}/lib64" ./configure --prefix=${TARGET_DIR} --disable-largefile --with-mpiexec=mpifort
make check
make install
