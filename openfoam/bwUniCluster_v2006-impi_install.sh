...
module load toolkit/oneAPI/compiler/2021.1.1
...
tar xvzf ThirdParty-${VERSION}.tgz --exclude=fftw-* --exclude=CGAL-* --exclude=ParaView-* --exclude=openmpi-* --exclude=boost_* && rm ThirdParty-${VERSION}.tgz
...
echo "export MPI_ROOT=/software/all/toolkit/Intel_OneAPI/mpi/2021.1.1" >> etc/config.sh/settings
...
sed -i s:'fftw_version=fftw-3.3.7':'fftw_version=fftw-3.3.9':g etc/config.sh/FFTW
