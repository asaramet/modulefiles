...
## Add in OpenFOAM-7/src/Pstream/Allwmake
  whichmpiFolder="$WM_PROJECT_DIR/platforms/$WM_OPTIONS/src/Pstream/$libName"
  [[ ! -d ${whichmpiFolder} ]] && mkdir -p ${whichmpiFolder}
## Before
  touch "$whichmpi"

## Add in OpenFOAM-7/src/parallel/decompose/Allwmake:
  whichscotchFolder="$WM_PROJECT_DIR/platforms/$WM_OPTIONS/src/parallel/decompose/$libName/using:$SCOTCH_VERSION"
  [[ ! -d ${whichscotchFolder} ]] && mkdir -p ${whichscotchFolder}
## before
